...
This skill ships with helper scripts and references under `skills/bs-sw-master/`:
...
**Path convention**: command examples below use paths relative to this skill's directory (e.g. `bash scripts/safety-snapshot.sh`). When invoking from a different cwd, prefix with the skill path — e.g. `bash skills/bs-sw-master/scripts/safety-snapshot.sh` if cwd is the repo root, or `bash ~/.claude/skills/bs-sw-master/scripts/safety-snapshot.sh` after installation.
