...
**Element budget:** target ≤9 primary elements (boxes/lifelines/states) per diagram; **exceeding 9 demands a recorded justification** (e.g. a deliberately exhaustive state table); hard ceiling 15 for skill-initiated choices; an explicit user instruction may exceed the ceiling as a recorded `USER-OVERRIDE` (see Rule 4). Members and edges carry load too: prefer ≤7 displayed members per class and treat ~25+ edges as a sign the diagram answers more than one question, even when the box count fits. Curation is the deliverable — a reverse-engineered diagram of every class is a failure even when accurate. Cut by: collapsing subtrees into one box, dropping getters/setters/constructors/DTO fields, excluding framework plumbing, splitting by concern.
