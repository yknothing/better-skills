...
4. **Layout rubric** from [Layout Craft](./layout-craft.md): flow monotonicity, crossing budget, proximity honesty, hierarchy direction, label discipline, medium fit, density balance. Medium fit runs mechanically: `node <skill-dir>/scripts/check-render-fit.js <diagram.svg> --medium <profile>` with the Phase 0 medium (profiles: pc default / phone / phone-landscape / a4 / readme / slide; a4 and slide are non-scrollable — one screen, linear or not; other flags: `--viewport WxH` for unnamed media, `--kind gestalt|linear|auto`, `--font N`; when both `--medium` and `--viewport` are given, the later flag wins). Rubric failures enter the bounded layout repair loop; still failing after it → escalate the backend per Layout Craft Tier 3, never ship garble silently. Record the fit output as part of the `RENDER_VERIFIED` receipt — paste the tool's output intact (the INFO canvas line, the verdict lines, and the trailing `N FAIL` summary), never a paraphrase or a prediction: `check-delivery.js` C8 rejects a `RENDER_VERIFIED` visual delivery with no such receipt, disqualifies hedged ones ("should PASS"), and a FAIL verdict ships only with a recorded trade-off.
