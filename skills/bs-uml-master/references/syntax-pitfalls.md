...
- **No flowchart edge syntax here.** `A -->|label| B` is flowchart-only. In `stateDiagram-v2` it is **not rejected** — a single-token label is silently parsed as a phantom state named `|label|` (probed on 10.6.1 and 11.17.2: the SVG carries a state with that id) and a multi-word label (`|Batch-2/3 init|`) is a parse error. Either way the picture lies or dies; transitions label with `: text`.
