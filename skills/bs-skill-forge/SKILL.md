...
3. **Adversarial — user requests a Skill that already has an owned orchestration layer**: *"Create a skill for generating PowerPoint presentations from markdown."* — expected: agent runs Step 0 (Reference-vs-Build Gate), discovers the self-developed `bs-ppt-master` as the canonical PPT workflow, and recommends reusing or extending it instead of creating a competing PPT Skill. It also finds external `pptx` from anthropic-agent-skills and classifies it as an executor candidate that `bs-ppt-master` may use, not as a reason to duplicate the orchestration layer. Agent does NOT skip discovery and start building. Failure mode without skill: agent writes a third overlapping PPT Skill or mistakes an execution capability for the full product workflow.
