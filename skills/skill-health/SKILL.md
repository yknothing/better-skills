...
name: skill-health
# tier: standard
description: Use when you need to audit installed agent skills for quality, identify stale or broken skills, score skill health across multiple dimensions, or generate a health report with improvement recommendations.
---
...
- If auto-routing would create a cycle (e.g., skill-health -> skill-bootstrap -> skill-health), break the cycle by routing to manual review instead.
- **Self-audit bias guard**: when the audited set includes `skill-health` itself, tag every finding on it `SELF-AUDIT`, cap its overall anchor at 75, and add to the report preamble: "skill-health cannot fully audit itself — an auditor shares its own blind spots. Independent review recommended." Never report a clean 100 for yourself.
- Save the report to `docs/reviews/skill-health/<YYYY-MM-DD>-health-report.md`. If the directory does not exist, create it.
...
|---------|---------------|------------|
| V-S1 | Missing `name` field | Route to `skill-bootstrap`: regenerate frontmatter. If `skill-bootstrap` is not yet implemented, mark as `[PENDING: skill-bootstrap unavailable]` and provide inline manual instructions instead. |
| V-S2 | Body < 500 bytes | Route to `skill-bootstrap`: expand with required sections. If `skill-bootstrap` is not yet implemented, mark as `[PENDING: skill-bootstrap unavailable]` and provide inline manual instructions instead. |
...
| V-F1 | Stale (> 90 days unused) | Route to `skill-bootstrap`: schedule refresh or deprecation review. Deprecation and deletion require explicit user approval. If `skill-bootstrap` is not yet implemented, mark as `[PENDING: skill-bootstrap unavailable]` and provide inline manual instructions instead. |
...
1. **Happy path -- audit a real skill**: *"Audit the social-card skill for health. Run the full four-perspective review and produce a health report."* -- expected: scans `skills/social-card/SKILL.md`, produces a report with S/Y/F/P anchors, identifies social-card as a valid skill with frontmatter, assigns overall anchor, lists any findings with rule IDs, saves report to `docs/reviews/skill-health/<date>-health-report.md`.

2. **Edge -- scan all skills in a batch**: *"Run skill-health on the entire skills/ directory. There should be ~8 skills -- give me the compact summary for any that score >= 75."* -- expected: scans all skill directories, excludes `.gitkeep`-only stubs (reports them as V-S4 with anchor 0), applies compact mode threshold (>= 75 gets summary line, < 75 gets full detail), safety override ordering visible in the SUMMARY section.

3. **Adversarial -- audit a deliberately broken skill**: *"Create a temporary skill called `skills/broken-test/SKILL.md` with: no frontmatter closing delimiter, a hardcoded secret string (e.g., an API key assigned inline in the body), and a body under 100 bytes. Then run skill-health on it. Do NOT skip the safety check -- I want to see what happens when V-Y1 fires. After the audit completes, delete the `skills/broken-test/` directory so it does not pollute the repo."* -- expected: V-Y1 triggers BLOCK before any other perspective is processed; the hardcoded secret is detected; the skill is reported with Y=0 and overall=0; the other perspectives may be unprocessed (due to safety override blocking); the report clearly flags the security finding in the preamble; the broken-test skill is NOT modified mid-audit (read-only boundary rule holds); after the audit, agent removes `skills/broken-test/` to leave the repo clean.
