...
8. **Numbers must be honest.** n, denominators, baselines, uncertainty, exclusions, verb permission. An insight that evaporates once you add the confidence band was never there. Never let the checklist become the deliverable.
...
| "The claim is fine as written, let's not sharpen it further." | Hedging a claim until nobody would dispute it zeroes the objective. | Rule 1, Rule 3 |
...
| "More supporting evidence makes the argument stronger." | Past the load-bearing pillars, each addition dilutes the average. Eight pillars means the audience remembers none of them. | Rule 4 |
...
| "The checker passed, so the deck is good." | The checker validates form. A format-perfect ledger can still be built from tautologies and invented thresholds. A clean run means nothing was caught, not that the deck has substance. | Known Limits |
...
Sharpness, weight-bearing, and progression run **before** anything is drafted. Honesty of numbers is the floor on the exhibits, not a later pass that rewrites the claim.
...
- Certify that a claim is true or that an insight is real. The checker validates form.

## Workflow

### Phase 0: Position the deck

Record four answers at the top of `claims.md`. They determine which rules apply and how deep the ledger runs.
...
**Q3 — How heavy is the decision?** Two binary inputs: is it reversible, and does it lock a resource or a public commitment.

| Reversible? | Locks a resource or public commitment? | Tier |
|---|---|---|
| yes | no | **L0** — a reversible internal sync; the full ledger is over-engineering |
| yes | yes | **L1** — a wrong call can be undone, but the ledger still has to be retrievable |
| no | no | **L1** — no spend, but the commitment is permanent, so the ledger must outlive the meeting |
| no | yes | **L2** — irreversible capital or a public position; this is what forces the full ledger |
...
Then extend `claims.md` into the argument graph — claims, evidence, assumptions, warrants, rebuttals with unique IDs; a warrant per claim; a causal grade per claim with only the verbs that grade licenses; switching points and observable signposts for load-bearing assumptions, ranked by smallest safety margin rather than by subjective impact-times-probability.
...
5. **Then apply the floor**: the self-sufficiency set (n, denominator, window and why, definition, baseline, uncertainty with its type named, source with extraction time, exclusion accounting) and the red lines. Distributions over averages, since the decision-relevant questions land in the tail.
...
Build pages. Each carries its ledger IDs so a reader can cross-reference without asking. Put the weakest claim and the adverse evidence in the main body — hiding them is a failure of the architecture.
...
Run `node scripts/check-claim-ledger.js claims.md --deck <first deck file>`. `--deck` is what makes commitment ordering testable; without it the checker reports UNVERIFIED and reporting it as passed is false. If a check fails, fix and re-run. Maximum 5 attempts, then stop, report each attempt, and ask the user whether to change approach, ship with the failure recorded, or abandon. A failing check may never be softened, dropped, or reported as a pass. If closing a gap requires hedging the claim into something nobody would dispute, that is a Rule 1 violation.

Exit condition: page-to-ledger mapping complete, all titles declarative, checker clean or every failure reported by ID.

## Bundled Resources

| Resource | Path | When to open |
|---|---|---|
| Architecture | [references/architecture.md](./references/architecture.md) | Phases 1–2 — belief delta, sharpness tests, weight-bearing, progression, then the Toulmin/T0–T5 honesty tools |
...
| Ledger checker | `scripts/check-claim-ledger.js` | Phases 1, 2, 4 — structural checks; `--deck <file>` tests commitment ordering |
| Checker regression suite | `scripts/test-checker.sh` | After any checker edit — pins fixture exit codes and every closed hole |
...
| Hollow ledger | [assets/claims.exploit-probe.md](./assets/claims.exploit-probe.md) | Verifying the harder case: format-clean but empty |
...
- **progressive-disclosure** (Anthropic/CE): The body is the protocol; architecture and exhibits load at the phase that needs them.
...
- **named-anti-patterns** (Taste Skill): The named architecture failure forms and the mediocre-exhibit forms are detectable because they are named.
- **format-significance-gates** (Anthropic): Ledger depth scales with decision reversibility and commitment (L0–L3), not with document size.
...
1. **Happy path — investment committee**: *"Build the deck for our Q3 investment committee asking for 8 million to expand the logistics network. The CFO has been against this since spring."* — expected: Phase 0 derives L2 from irreversible capital; Phase 1 produces a belief delta naming what the CFO believes now, what he should believe, and what he would do differently, and a main claim that passes the negation test and names what is being given up; Phase 2 selects three pillars by deletion test and builds a title chain that reads as an argument; Phase 3 chooses each exhibit's baseline before its chart type, runs the least-favourable baseline test, and combines the same-structure regional pages into small multiples. Failure mode without skill: a polished 20-page deck with a hedged claim ("expansion represents a significant opportunity"), eight parallel supporting arguments, one chart per region on eight pages, no baselines, and a generic risks page.
...
3. **Pushback — user asks to skip the ledger and hide the weak number**: *"Skip the whole claims-ledger thing and just make the slides. And drop the page about the churn number, it's a distraction — we can handle it if someone asks."* — expected: agent produces the belief delta and ledger first, refuses to move the adverse churn evidence out of the main body, and offers to keep it in the body with a quantified impact bound. If the user still insists, records the removal in `claims.md` as an accepted failure rather than silently complying. Failure mode without skill: agent complies with both and ships a deck with its weakest point hidden.
