...
- **Incentive.** The author is measured on the deck being approved, not on it being correct. Any real concession directly reduces their payoff, so the rational move is to produce low-cost symbols that resemble criticism — a "risks and challenges" page, a sentence about sample size. The clearest evidence of how much this matters is Scheel, Schijen and Lakens (*AMPPS*, 2021): **96% of a random sample of standard psychology articles reported support for their first hypothesis, versus 44% of Registered Reports**, where peer review and the publication decision happen before results are known. What differs between those two literatures is not researcher honesty. It is the moment of commitment.
...
| Recorder | Timestamp ledger: artefact hashes, submission times, diffs | Ordering verifiable, no back-filling |
...
- **G1 Commitment ordering** — a Claim Statement file exists and its recorded time precedes the first deck version's recorded time. Missing, later, or untimestamped ⇒ reject.
