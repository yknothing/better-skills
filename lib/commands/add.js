...
function run({ _, flags }) {
  ...
  if (dryRun) {
    ...
    const files = installer.copyTree(absSourceDir, destDir, { dryRun: true, canonicalName: name, aliases: src.aliases });
  ...
  let files;
  try {
    files = installer.copyTree(absSourceDir, destDir, { canonicalName: name, aliases: src.aliases });
  } catch (e) {
