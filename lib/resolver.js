...
function canonicalizeName(name, skillsJson) {
  const registry = skillsJson || loadRegistry().skillsJson;
  return registry.aliases?.[name] || name;
}

function legacyNamesFor(canonicalName, skillsJson) {
  const registry = skillsJson || loadRegistry().skillsJson;
...
// Returns canonical name plus the originally requested name.
// Throws ENOTFOUND if neither the name nor its alias is registered.
