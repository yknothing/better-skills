# Advocate Review: skill-health

**Reviewer role**: Advocate (Gate 2 — Peer Review)
**Date**: 2026-06-17
**Skill under review**: `skills/skill-health/SKILL.md`
...
|------|---------|------------|
| V-S1 (missing name) | skill-bootstrap | Correct |
| V-S2 (body < 500 bytes) | skill-bootstrap | Correct |
...
| V-F1 (stale > 90 days) | skill-bootstrap | Reasonable |
...
- **V-S6**: Missing or malformed frontmatter closing delimiter -> Route to `skill-bootstrap`: fix YAML frontmatter syntax
- **V-S7**: Description does not start with "Use when" -> Route to `skill-bootstrap`: rewrite description to follow CSO convention
...
This is a meta-circular check that asks the skill-health auditor to audit the skill-health SKILL.md itself. The question is whether this is practical insight or empty recursion.

**Arguments for practicality**:

1. The skill-health SKILL.md is a SKILL.md file like any other. It has frontmatter, a body, claims pattern usage, and has a file modification date. All four perspectives can be meaningfully applied to it.

2. Self-application could catch real issues: Does skill-health's own frontmatter pass the structural checks? Are its claimed patterns properly attributed in `docs/patterns/README.md`? Is its body within the 500-5000 word range? (At 1,746 words, it is.)

3. For a meta-skill, the ability to audit itself is a proof of correctness. If skill-health cannot pass its own checks, something is wrong with either the skill or the checks.
...
2. If self-application finds issues that require fixing the skill-health SKILL.md, this creates a recursive loop: fix skill-health, re-audit with the fixed version, potentially find new issues, repeat. The boundary rule about cycle detection addresses routing cycles but not self-modification cycles.
...
6. **Cycle detection**: If auto-routing would create a cycle (skill-health -> skill-bootstrap -> skill-health), break by routing to manual review. This prevents infinite routing loops.

7. **Report save path**: `docs/reviews/skill-health/<YYYY-MM-DD>-health-report.md` with directory creation if needed. This ensures the report has a predictable, versioned location.

**Gaps identified**:

1. **No rule for existing report file**: What happens if `docs/reviews/skill-health/2026-06-17-health-report.md` already exists? Overwrite? Append? Create a suffixed version? This should be specified. Running the same audit twice on the same day is a realistic scenario (e.g., re-running after fixes).
...
|---------------|-------------------|-------------------|
| Missing frontmatter closing `---` delimiter | V-S6 | Route to `skill-bootstrap`: fix YAML frontmatter syntax |
| Description does not start with "Use when" | V-S7 | Route to `skill-bootstrap`: rewrite description per CSO convention |
