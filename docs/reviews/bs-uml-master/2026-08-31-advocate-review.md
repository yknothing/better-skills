...
R4 converts the last major judgment call in this skill — "does the layout serve the reader?" — into a mechanical, dependency-free gate (`check-render-fit.js`), and the two rebind rounds (R4.1 and this revision's roledescription fix) repaired the gate's real defects against actual mermaid 11 output: I independently reproduced the worst one (the v1 edge regex matched 0 of the real edges because mermaid emits `d=` before `class=`; the v2 order-independent parser matches 5/5 on the same file) and confirmed the F3 residual — flowchart `classDef actor` markup flipping kind detection — is closed by roledescription-first detection whose probed fixture value (`flowchart-v2`) I matched against the real probe SVGs. The skill is now on its sixth empirically-driven iteration, each traceable through a 20-entry improvement-points ledger from observed failure to specific enforcement, with the rebinds themselves demonstrating that the adversary/advocate pipeline catches what synthetic self-tests miss. I would ship this: every claim I tested held up under rerun.

## Evidence Reviewed

Full manifest receipt acknowledged: a2dc6724fdcde28d6bdebff5b7c76b2f386ae4faad51045145bf0b3080136994
...
- `skills/bs-uml-master/scripts/check-render-fit.js` (SHA verified: dff57440…) — read line by line and diffed across v1→v2→v3: order-independent tag/attribute parsing, `<line>` sequence-message support, sequence-forces-vertical reading axis, audit WARN on manual linear without markers, and (this revision) `aria-roledescription` as the authoritative kind signal with class-marker fallback for older renderers — which also let the mathematically unreachable gestalt long-edge FAIL branch be removed with an explanatory comment
- `skills/bs-uml-master/scripts/test-check-render-fit.js` (SHA verified: 65176b32…) — 14 fixtures; 9–13 encode the adversary's findings (d-before-class, line edges, wide-sequence overflow, real captured `classDef actor` flowchart markup, probed `aria-roledescription="sequence"`, laundering WARN)
...
- `git rev-parse HEAD` → `9986df3a6fbeb54149a457586d5f035ee3cf4843` (matches Reviewed Revision); diffs `2b94700..2c78b61` and `2c78b61..9986df3` inspected in full for the scoped files
...
- `node skills/bs-uml-master/scripts/test-check-render-fit.js` → ALL PASS (14 fixtures), including `edge-d-before-class`, `sequence-line-edges`, `wide-sequence-overflow`, `classdef-actor-real-markup`, `roledescription-sequence`, and `manual-linear-laundering-warn`
...
- `node skills/bs-uml-master/scripts/check-render-fit.js` (current v3) on the four real probe SVGs from the Haiku #2 review: `d1.svg` → FAIL (6.5px, aspect 0.40:1 WARN), `d2.svg` → FAIL (7.0px), `d1-elk.svg` → PASS (14.5px), `d3.svg` → PASS (12.0px) — the real-SVG regression is unchanged across both reworks and all four verdicts still match the human judgments recorded in the usage review
- `grep aria-roledescription` on the real probe SVGs → `flowchart-v2` on d1/d3, confirming fixture 12's captured markup is the authentic mermaid 11 value the roledescription-first fix keys on
...
| Hard rules / safety gates | 9/10 | Three well-formed HARD-GATE tags; two deterministic checkers with in-repo regression suites; receipt-bearing state vocabulary; the rebinds closed the kind-laundering, wrong-viewport, and classDef-spoof holes | Fit gate still self-attested — check-delivery.js does not demand a fit receipt (IP-20, open) |
...
| Bundled resources | 9/10 | 10/10 exist; both checkers dependency-free Node with self-tests; fit checker is calibrated against real mermaid 11 output (5/5 edges parsed where v1 parsed 0/38; roledescription values probed and pinned as fixtures) and its 14 fixtures now include captured real markup, not just the checker's own assumptions; the dead gestalt long-edge branch was removed rather than left as latent complexity | The v1 edge gate shipped inert — synthetic fixtures had mirrored the checker's parsing assumptions; the risk class is now known but recurrence depends on continued real-output probing |
...
This revision is the strongest of an already unusually well-evidenced skill, and the rebind rounds are themselves evidence the quality process works: an adversary probing real renderer output found the edge gate inert, the axis heuristic launderable, and a live classDef-spoof residual, and each defect was fixed same-day, fixture-encoded (14-fixture suite, ALL PASS, including captured real mermaid markup), ledgered (IP-17/18/19 fixed, IP-20 honestly left open), and the real-SVG regression re-verified unchanged after every rework. Nothing important here is asserted on authority: the layout recipe's negative results are recorded, the fit gate's thresholds reproduce exactly on the renders that motivated them, I reproduced the 0-matches-to-5-matches edge-parsing fix and the `flowchart-v2` roledescription value myself on the real probe files, Gate 1 passes 16/16, and both self-test suites pass. The residual risks — self-attested fit receipts, agent-dependent HTML mirror discipline — are known, ledgered, and scheduled rather than hidden, which is precisely the honesty behavior this repo's review pipeline exists to produce.
